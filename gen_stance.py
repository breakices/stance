...
MODEL_PATH = "/online1/sc100123/sc100123/agentic_moderation/LLaMA-Factory/saves/qwen3-8b/full/sft_cot/checkpoint-141/"
...
def make_jsonable(obj):
    ...
    Handles numpy types, pandas Timestamps, and nested structures.
    """
...
def main(temperature: float, top_p: float) -> None:
    ...
    # Build a lightweight JSON-serializable list:
    # only keep id, target_text, gold label, and cot.
    lite_records = []
    for rec, out in zip(records, outputs):
        # adapt this key name if your id column is different (e.g., "id" instead of "unit_id")
        unit_id = rec.get("unit_id", None)
        target_text = rec.get("target_text", "")
        gold_label = rec.get("label", "")

        # Make sure everything is JSON-friendly (cast to str or int)
